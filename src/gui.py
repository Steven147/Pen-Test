...
class MainWindow(QMainWindow):
    def __init__(self, parent=None, movable=False):
        ...
        exitAct = QAction(QIcon('.\icon\exit_472px_1179234_easyicon.net.png'), 'Exit', self) #新建exit action
        exitAct.setShortcut('Ctrl+Q') #命名
        exitAct.setStatusTip('Exit application') #状态栏
        exitAct.triggered.connect(self.close) #绑定close 动作
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_Return:  ### 16777220:  # This is the ENTER key
            ...
            if text == self.name + text.replace(self.name, "") and text.replace(self.name, "") != "":  # This is to prevent adding in commands that were not meant to be added in
                self.commands.append(text.replace(self.name, ""))# print(self.commands)
            self.handle(text)
